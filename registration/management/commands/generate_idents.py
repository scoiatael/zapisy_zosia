...
def generate_meals(preference):
    ...
    if preference.dinner_1:
        result += u'{ Czwartek -- obiad, 20:00-21:30}'
    else:
        result += u'{}'
    if preference.breakfast_2:
        result += u'{ Piątek -- śniadanie, 8:00-9:30}'
    else:
        result += u'{}'
    if preference.dinner_2:
        result += u'{ Piątek -- obiad, 17:00-19:30}'
    else:
        result += u'{}'
    if preference.breakfast_3:
        result += u'{ Sobota -- śniadanie, 8:00-9:30}'
    else:
        result += u'{}'
    if preference.dinner_3:
        result += u'{ Sobota -- obiad, 17:00-19:30}'
    else:
        result += u'{}'
    if preference.breakfast_4:
        result += u'{ Niedziela -- śniadanie, 8:00-9:30}'
    else:
        result += u'{}'
